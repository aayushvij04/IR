...
Running the core Python implementations on our sample datasets validates the theoretical models:
\begin{itemize}[leftmargin=*, nosep]
  \item \textbf{Boolean \& Compression:} The inverted index correctly resolves complex queries (\texttt{index AND NOT boolean}). Variable-Byte encoding successfully compressed a sample postings list down to 8 bytes. Heaps' law estimated a 44,000 term vocabulary for 1M tokens.
  \item \textbf{Ranking Models:} BM25 out-performs standard VSM on the test queries. The custom evaluation module reported a \textbf{MAP of 0.491}, \textbf{MRR of 0.500}, and \textbf{nDCG@5 of 0.960}.
  \item \textbf{Language Models:} Dirichlet Smoothing ($\mu=2000$) efficiently mitigated zero-frequency terms, improving the query likelihood scores over basic Jelinek-Mercer.
  \item \textbf{Web Search:} PageRank converged in 27 iterations on the simulated web graph, and HITS correctly identified the primary authoritative nodes.
\end{itemize}
